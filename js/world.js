var world = function() {
	...
	var init = function(level, canvasId, hudId, tipId) {
		...
		initLevel(level);
		createDialogue("Welcome to Swap by BSD Academy");
		prevTime = Date.now();
	...
	var initLevel = function(level) {
		...
		curLevel = level;
		alert("Starting time: " + new Date().getTime());
	...
	var victory = function() {
		if(!hasWon) {
			...
			curLevel++;
			alert("You have made it to the next level. Your time for this level is: " + new Date().getTime());
		}
		hasWon = true;
	}
